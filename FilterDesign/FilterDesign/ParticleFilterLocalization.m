...
MAX_RANGE=30;%20;%最大観測距離
NP=100;%パーティクル数
NTh=NP/2;%リサンプリングを実施する有効パーティクル数(ユーザの指定)
%ESS有効サンプルサイズ
...
for i=1 : nSteps
    ...
    %output:
    %z:基準点からの距離: MAX_RANGEを超えるとRFIDからの値は返ってこない
    %xtrue:移動距離
    %xd:デッドレコニング値
    %u:観測値
    ...
    for ip=1:NP
        ...
        x=f(x, u)+sqrt(Q)*randn(3,1); %xtの推定
        %信号にノイズを加えてみようQ:既知
        %過去の状態からの繊維とシステムノイズの人為的な混入
        
        % Calc Inportance Weight
        for iz=1:length(z(:,1))
            pz=norm(x(1:2)'-z(iz,2:3)); %推定状態からの観測値を返す
            %pz:基準点からランダムサンプリング点(DR)までの距離
            dz=pz-z(iz,1); 
            %dz:"基準点からランダムサンプリング点(DR)までの距離"と"基準点からの距離"との差
            w=w*Gauss(dz,0,sqrt(R));
            %pdf('Normal',dz,0,sqrt(R))
            %R:実際の観測時に乗るノイズ
            %標準正規分布による計算
            %分散
            %尤度の算出
...
Neff=1.0/(pw*pw'); %ESS
if Neff<NTh %リサンプリング
    wcum=cumsum(pw); % 累積和.徐々に足す
    base=cumsum(pw*0+1/NP)-1/NP;%乱数を加える前のbase 重み 定数?
    resampleID=base+rand/NP;%ルーレットを乱数分増やす rand/NPはMAXで1/NP
    ppx=px;%データ格納用
    ind=1;%新しいID
    for ip=1:NP
...
u=u+sqrt(Qsigma)*randn(2,1);%add Process Noise %観測値
