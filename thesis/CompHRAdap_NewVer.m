%心拍数と脈拍数の比較プログラム. 一切, パラメータのチューニング等を行わない.
...
figure;
plot(F,Cxy);
title('Magnitude-Squared Coherence X Axis Acceleration, Roll Angular Speed','FontSize',FontSize);
...
figure;
plot(F,Cxy);
title('Magnitude-Squared Coherence X Gyro Angular Speed, Roll Angular Speed','FontSize',FontSize);
xlabel('Frequency (Hz)','FontSize',FontSize);
set(gca,'FontSize',FontSize);
grid;
coheFreqRange = [0.7 3.0];
...
figure;
plot(F,Cxy);
title('Magnitude-Squared Coherence X Axis Acceleration, X Gyro Angular Soeed','FontSize',FontSize);
xlabel('Frequency (Hz)','FontSize',FontSize);
grid;
set(gca,'FontSize',FontSize);
coheFreqRange = [0.7 3.0];
