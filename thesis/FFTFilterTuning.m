%PFBLMSアルゴリズムのチューニング及び脈拍数の評価を行うプログラム
%末尾部でエラー出るかも.
%エラーが出た場合はワークスペースを保存後, FFTResDisp.mで結果の表示を行ってください.
...
for trialIndex = 1 : trialLength
    ...
    for filteCoeffIndex = 1:searchFilterCoefLengthProcNum
        for FFTLMSStepSizeIndex = 1:FFTLMSStepProcNum
            FFTLMSFilter = dsp.FrequencyDomainAdaptiveFilter('Length',searchFilterCoefLength(filteCoeffIndex),...
                'StepSize',FFTStepSizeArray(FFTLMSStepSizeIndex),'Method','Partitioned constrained FDAF');
            disp(strcat('FilterOrder:',num2str(searchFilterCoefLength(filteCoeffIndex))));
            disp(strcat('FFTLMS StepSize:',num2str(FFTStepSizeArray(FFTLMSStepSizeIndex))));
            ...
            for axisIndex = 1:Dict.Count
                ...
                if axisIndex < TriAccKey
                    [~,adaptOutput] = FFTLMSFilter(PPGDataArray(:,trialIndex),...
                    ...
                    FFTLMSRMSEArray(trialIndex,filteCoeffIndex,FFTLMSStepSizeIndex,axisIndex) = adaptPulseRateError;
                    FFTLMSFilter = dsp.FrequencyDomainAdaptiveFilter('Length',searchFilterCoefLength(filteCoeffIndex),...
                    'StepSize',FFTStepSizeArray(FFTLMSStepSizeIndex),'Method','Partitioned constrained FDAF');
                else
...
FFTLMSRMSEMean = zeros(trialLength,searchFilterCoefLengthProcNum,FFTLMSStepProcNum,length(evalAxis));
disp('FFTLMSアルゴリズムによる評価結果');
for trialIndex = 1 : trialLength
    otherDataIndex = 1:1:trialLength;
    otherDataIndex(trialIndex) = '';
    FFTLMSRMSEMean(trialIndex,:,:,:) = mean(FFTLMSRMSEArray(otherDataIndex,:,:,...
        evalAxis));
    %各軸について最小を評価
    for axisIndex = 1 : length(evalAxis)
        curRMSEMean = reshape(FFTLMSRMSEMean(trialIndex,:,:,axisIndex),...
        [searchFilterCoefLengthProcNum FFTLMSStepProcNum]);
        ...
        'ステップサイズ:',num2str(FFTStepSizeArray(StepInd)),''));
        disp(strcat(num2str(trialIndex),'個目のデータのRMSE:',num2str(FFTLMSRMSEArray(trialIndex,coefLenInd,StepInd,evalAxis(axisIndex)))));
    end
end
