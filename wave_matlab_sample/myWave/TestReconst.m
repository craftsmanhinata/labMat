...
inputSignal = 2*sin(2*pi*0.6801*time)+2*sin(2*pi*10.8819*time)+3*sin(2*pi*37.8929*time.*(time>5.0));
...
imagesc(time,(frequencyWavelet),(waveletSignalPower./varInputSignal));
set(gca,'YDir','normal');
colorbar();
title('Input Signal Scaleogram');
xlim([0,(point-1)*samplingPeriod]);
xlabel('Time(Sec)');
ylabel('Wrong Frequency(Hz)');
