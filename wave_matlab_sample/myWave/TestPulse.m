...
inputSignal = 2*sin(2*pi*0.55243*time)+2*sin(2*pi*0.8*time)+3*sin(2*pi*2*time.*(time>5.0));
...
title('Reconstructed Signal Time Space');
xlabel('Time(Sec)');
xlim([0,(point-1)*samplingPeriod]);
ylim([signalMin,signalMax]);
...
title('Difference input signal to reconstructed signal');
xlabel('Time(Sec)');
xlim([0,(point-1)*samplingPeriod]);
...
title('Noise removed reconstructed signal');
xlabel('Time(Sec)');
xlim([0,(point-1)*samplingPeriod]);
ylim([signalMin,signalMax]);
